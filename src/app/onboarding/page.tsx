...
import {
  ArrowRight, ArrowLeft, Check, Home, Dumbbell, Cable, Weight, Loader2, AlertTriangle,
} from "lucide-react";
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-30 glass border-b border-border">
        <div className="mx-auto flex max-w-2xl items-center justify-between px-4 py-3">
          <button type="button" onClick={prev} disabled={step === 0 || step === 3}
        ...
        <div className="fixed inset-x-0 bottom-0 z-30 glass border-t border-border safe-bottom">
          <div className="mx-auto max-w-2xl p-4 space-y-2">
            <button type="button" onClick={generating ? cancelGeneration : next}
              ...
              {generating ? <><Loader2 className="h-4 w-4 animate-spin" /> Annuler</>
  ...
  return (
    <div className="animate-slide-up space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        {(["age", "height", "weight", "goalWeight"] as const).map((key) => (
          <label key={key} className="block">
            <span className="mb-1.5 block text-xs font-medium text-muted-foreground">
              {key === "age" ? "Âge" : key === "height" ? "Taille (cm)" : key === "weight" ? "Poids (kg)" : "Poids objectif (kg)"}
            </span>
            <SimpleInput type="number" step={key === "age" ? 1 : 0.1}
              value={(form[key] as number) || ""}
              onChange={(e) => update(key, e.target.value === "" ? 0 : key === "age" ? parseInt(e.target.value) : parseFloat(e.target.value))} />
          </label>
        ))}
      </div>
    ...
    <div className="animate-slide-up space-y-4">
      ...
      {[{ id: "gain" as const, emoji: "💪", title: "Prise de masse", sub: "+200 à +500 kcal" },
        ...
        <button key={g.id} type="button" onClick={() => update("goal", g.id)}
          ...
          style={{ borderColor: form.goal === g.id ? "var(--accent)" : "var(--border)", background: form.goal === g.id ? "color-mix(in oklab, var(--accent) 10%, var(--surface-1))" : "var(--surface-1)" }}>
          {form.goal === g.id && <span className="absolute left-0 top-0 h-full w-1 grad-accent" />}
