  ...
  const toggleDone = async (exId: number) => {
    ...
    if (sessionId) {
      const updatedExercises = workout.exercises.map((e) =>
        e.id === exId
          ? { name: e.name, sets: e.sets, reps: e.reps, rest: e.rest, done: !done[exId] }
          : { name: e.name, sets: e.sets, reps: e.reps, rest: e.rest, done: done[e.id] }
      );
      await supabase.from("workout_sessions").update({ exercises: updatedExercises, completed: Object.values(newDone).every(Boolean) } as Record<string, unknown>).eq("id", sessionId);
    }
  };
