...
import { adaptiveProgressionEngine } from "@/lib/adaptive-engine";
import type { AdjustedPlan } from "@/lib/adaptive-engine";
...
function Progress() {
  ...
  const chartData = weightHistory.map((w) => ({
    date: w.logged_date?.slice(5) ?? "", w: w.weight_kg,
  }));
  ...
  return (
    <AppShell header={<PageHeader title="Progress" subtitle={tab === "Check-in" ? "Check-in hebdomadaire" : `${avgKcal} kcal/j moy`} />}>
            ...
            <div className="mt-3 h-44">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={{ left: -20, right: 8, top: 10, bottom: 0 }}>
                  <defs><linearGradient id="w" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="var(--accent)" stopOpacity={0.4} /><stop offset="100%" stopColor="var(--accent)" stopOpacity={0} /></linearGradient></defs>
      ...
      {tab === "Check-in" && <WeeklyCheckin userId={userId} dbProfile={dbProfile} />}
    </AppShell>
  );
}

function WeeklyCheckin({ userId, dbProfile }: { userId: string | undefined; dbProfile: Record<string, unknown> | null }) {
  ...
  const [result, setResult] = useState<AdjustedPlan | null>(null);
  ...
  const handleCheckin = async () => {
    ...
    try {
      ...
      const goal = (dbProfile as Record<string, string> | null)?.goal ?? "maintain";
      const currentKcal = caloriesAvg ? parseInt(caloriesAvg) : (dbProfile as Record<string, number> | null)?.target_kcal ?? 2000;

      const adjustment = adaptiveProgressionEngine({
        currentWeight: parseFloat(weight),
        previousWeight,
        targetGoal: goal as "loss" | "gain" | "maintain",
        fatigueScore,
        adherenceScore,
        workoutPerformance: workoutPerf,
        currentKcal,
        currentProtein: 0, currentCarbs: 0, currentFat: 0,
        sleepScore,
        motivationScore,
      });
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-border bg-surface-1 p-4">
        ...
        <p className="mt-1 text-xs text-muted-foreground">Remplis tes stats pour obtenir des ajustements IA personnalisés</p>
      ...
      {result && (
        <div className="rounded-2xl border border-accent/30 bg-surface-1 p-4 animate-slide-up">
          <h3 className="text-sm font-semibold flex items-center gap-2" style={{ color: "var(--accent)" }}><Sparkles className="h-4 w-4" />Ajustements IA</h3>
          ...
          {result.cardioSessions > 0 && (
            <div className="mt-2 rounded-lg bg-surface-2 px-3 py-2 text-xs">🏃 Cardio : +{result.cardioSessions} séance(s)/semaine</div>
          )}
          {result.volumeAdjustment !== 0 && (
            <div className="mt-2 rounded-lg bg-surface-2 px-3 py-2 text-xs">
              🏋️ Volume : {result.volumeAdjustment > 0 ? "+" : ""}{result.volumeAdjustment}%
            </div>
          )}
          {result.deload && (
            <div className="mt-2 rounded-lg px-3 py-2 text-xs font-semibold" style={{ background: "color-mix(in oklab, var(--warning) 15%, transparent)", color: "var(--warning)" }}>
              🛑 Semaine de décharge recommandée
            </div>
          )}
          {result.intensityModifier !== 0 && (
            <div className="mt-2 rounded-lg bg-surface-2 px-3 py-2 text-xs">⚡ Intensité : {result.intensityModifier > 0 ? "+" : ""}{result.intensityModifier}%</div>
          )}
          {result.restModifier !== 0 && (
            <div className="mt-2 rounded-lg bg-surface-2 px-3 py-2 text-xs">⏱️ Repos : {result.restModifier > 0 ? "+" : ""}{result.restModifier}s entre séries</div>
          )}
  ...
  return (
    <div>
      ...
      <input type="range" min={0} max={10} value={value} onChange={(e) => onChange(Number(e.target.value))}
        className="mt-1 w-full accent-[color:var(--accent)] h-2" style={{ accentColor: barColor }} />
    </div>
  );
}
