...
import {
  Sparkles, Send, Trash2, Bot, User, ArrowLeft, BarChart3, Utensils, Pill, Dumbbell,
} from "lucide-react";
...
export const Route = createFileRoute("/chat")({
  head: () => ({
    meta: [
      { title: "Coach FitAI — Chat" },
      { name: "description", content: "Pose tes questions à ton coach IA fitness & nutrition." },
    ],
  }),
  component: Chat,
});
...
function Chat() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content: `Salut **${name}** 👋 Je suis ton Coach FitAI. Pose-moi toutes tes questions sur la nutrition, l'entraînement, ou ta progression aujourd'hui !`,
    },
  ]);
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      const reply = await askCoach(
        allMessages,
        { name, goal, mode, target_kcal: macros.kcal, target_protein: macros.protein },
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: "Désolé, je n'arrive pas à contacter le serveur pour le moment. Vérifie que ta clé API DeepSeek est configurée dans .env.local et réessaie." },
      ]);
    } finally {
      setTyping(false);
    }
      ...
      header={
        <header className="sticky top-0 z-30 glass border-b border-border">
          <div className="mx-auto flex max-w-2xl items-center justify-between px-4 py-3">
            <Link to="/" className="grid h-9 w-9 place-items-center rounded-full bg-surface-2">
              <ArrowLeft className="h-4 w-4" />
            </Link>
            <div className="flex items-center gap-2">
              <span className="grid h-7 w-7 place-items-center rounded-full grad-accent">
            ...
            <button onClick={clear}
              className="grid h-9 w-9 place-items-center rounded-full bg-surface-2 text-muted-foreground hover:text-destructive">
      ...
      <div className="flex flex-col gap-3 pb-4">
          ...
          <div key={i} className={`flex gap-2 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
            ...
            <div className="max-w-[85%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed"
              style={msg.role === "assistant"
                ? { background: "var(--surface-2)", borderTopLeftRadius: 4 }
                : { background: "color-mix(in oklab, var(--accent) 10%, transparent)", border: "1px solid color-mix(in oklab, var(--accent) 25%, transparent)", borderTopRightRadius: 4 }}>
        ...
        {typing && (
          <div className="flex gap-2">
            <span className="mt-1 grid h-7 w-7 shrink-0 place-items-center rounded-full grad-accent text-background">
