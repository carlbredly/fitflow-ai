...
/**
 * Extracts and parses the first valid JSON object from a potentially
 * markdown-wrapped or padded model response.
 */
...
/**
 * Validates and clamps a FoodItem returned by the model.
 * Prevents NaN / negative values from polluting the journal.
 */
...
// ─── Image Analysis (DeepSeek vision) ────────────────────────────────────────
// deepseek-chat supports vision via the OpenAI-compatible endpoint.
// Image is passed as a base64 data URL in the content array.
  ...
  const res = await fetch("https://api.deepseek.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: `Tu es un nutritionniste expert et diététicien clinique avec 20 ans d'expérience.
Tu analyses des photos de repas pour en extraire la composition nutritionnelle précise.

RÈGLES STRICTES :
1. Réponds UNIQUEMENT avec un objet JSON valide, sans texte avant ou après, sans backticks.
2. Identifie CHAQUE composant visible individuellement (viande, légume, sauce, accompagnement, etc.).
3. Estime les quantités en grammes selon les proportions visuelles dans l'assiette.
4. Pour chaque aliment, indique ta confiance ("haute" si clairement identifiable, "moyenne" si probable, "faible" si incertain).
5. Les macros DOIVENT être cohérentes : vérifie que kcal ≈ protéines*4 + glucides*4 + lipides*9.
6. Si l'image n'est pas un repas, retourne {"erreur": "image_invalide"}.

FORMAT REQUIS (JSON pur, aucun texte autour) :
{
  "aliments": [
    {
  ...
  "notes": "Observation qualitative du repas en une phrase."
}`,
        },
        {
          role: "user",
          content: [
            ...
            {
              type: "text",
              text: "Analyse ce repas et retourne uniquement le JSON nutritionnel. Sois précis sur les portions en évaluant la taille de l'assiette comme référence visuelle. JSON pur, sans backticks.",
            },
          ],
        },
      ],
  ...
  const parsed = extractJSON<ScanResult & { erreur?: string }>(text);
  if (parsed.erreur === "image_invalide") throw new Error("L'image ne contient pas de repas reconnaissable.");

  // Sanitize each food item
  parsed.aliments = (parsed.aliments ?? []).map(sanitizeFoodItem);

  // Recompute total server-side to avoid model arithmetic errors
  parsed.total = parsed.aliments.reduce(
    (acc, a) => ({
      kcal:        acc.kcal        + a.kcal,
      proteines_g: acc.proteines_g + a.proteines_g,
      glucides_g:  acc.glucides_g  + a.glucides_g,
      lipides_g:   acc.lipides_g   + a.lipides_g,
    }),
...
// ─── Meal Plan Generation (DeepSeek text) ────────────────────────────────────
...
async function generateMealPlan(params: {
  ...
  const goalLabel =
    params.goal === "gain" ? "prise de masse musculaire"
    : params.goal === "loss" ? "perte de poids / déficit calorique"
    : "maintien du poids";
  ...
  const res = await fetch("https://api.deepseek.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: `Tu es un diététicien-nutritionniste clinique spécialisé en nutrition sportive.
Tu crées des plans alimentaires journaliers précis, savoureux et réalistes.

RÈGLES ABSOLUES :
1. Réponds UNIQUEMENT avec un objet JSON valide, sans texte autour, sans backticks, sans markdown.
2. Les repas doivent être équilibrés et appétissants, pas des combinaisons arbitraires.
...
6. Répartis les protéines uniformément sur tous les repas (principe d'anabolisme optimal).
7. Breakfast doit être nourrissant mais digeste. Dinner plus léger en glucides.

FORMAT REQUIS (respecter exactement ces clés, JSON pur) :
{
  "repas": [
    {
      "type": "breakfast",
...
OBJECTIF : ${goalLabel}
MODE : ${params.mode === "strict" ? "Strict (aliments propres, pas de junk food)" : "Flexible (quelques extras autorisés)"}

MACROS CIBLES EXACTES :
- Calories : ${params.targetKcal} kcal
- Protéines : ${params.targetProtein}g
- Glucides : ${params.targetCarbs}g
- Lipides : ${params.targetFat}g

Répartis sur : breakfast (25%), lunch (35%), dinner (30%), snack (10%).
Priorise des sources protéiques de qualité (viandes maigres, œufs, légumineuses, produits laitiers).
JSON pur uniquement, aucun texte autour.`,
  ...
  const parsed = extractJSON<MealPlanResult>(text);

  // Validate and sanitize
  if (!Array.isArray(parsed.repas)) throw new Error("Format de plan invalide");
  parsed.repas = parsed.repas.map((r) => ({
    ...r,
  ...
  // Recompute total
  parsed.total = parsed.repas.flatMap((r) => r.aliments).reduce(
    (acc, a) => ({
      kcal:        acc.kcal        + a.kcal,
      proteines_g: acc.proteines_g + a.proteines_g,
      glucides_g:  acc.glucides_g  + a.glucides_g,
...
// ─── Food Search (DeepSeek text) ──────────────────────────────────────────────

async function searchFoodAI(query: string): Promise<Array<{
  name: string; per: string; kcal: number; p: number; c: number; f: number;
}>> {
  const res = await fetch("https://api.deepseek.com/v1/chat/completions", {
    method: "POST",
    headers: {
...
Règles : 4-6 résultats max. kcal cohérent avec macros (kcal ≈ p*4 + c*4 + f*9). Variantes pertinentes uniquement.`,
  ...
  try {
    ...
    const arr = JSON.parse(stripped) as Array<{ name: string; per: string; kcal: number; p: number; c: number; f: number }>;
  ...
  return (
    <AppShell
      header={
        <PageHeader
          title="Nutrition"
          subtitle={`${totals.kcal} / ${macros.kcal} kcal aujourd'hui`}
        />
      }
    >
...
function JournalView({
  ...
  addFood: (f: { food_name: string; meal_type: MealType; quantity_g?: number; kcal?: number; protein_g?: number; carbs_g?: number; fat_g?: number; source?: string }) => void;
  isAdding: boolean;
  ...
  return (
    <>
            ...
            return (
              <div key={macro} className="flex justify-between">
                ...
                <span className="font-mono">
                  {Math.round(totals[macro])}/{macros[macro]}g
                </span>
              </div>
            );
          })}
      ...
      <button
        ...
        disabled={aiLoading || isAdding || !dbProfile}
      ...
      {aiStatus && (
        <div
          ...
          style={{
            ...
            color: aiStatus.type === "success" ? "var(--accent)" : "#ef4444",
          }}
        >
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <span
                    ...
                    style={{ color: kcal > 0 ? "var(--accent)" : "var(--muted-foreground)" }}
                  >
                    {kcal} kcal
                  </span>
                  <ChevronDown
                  ...
                  {isAddingHere ? (
                    <div className="space-y-2 py-2">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => submitFood(type)}
                          disabled={!foodName.trim() || isAdding}
      ...
      {/* FAB */}
      <button
        onClick={() => { setOpen("lunch"); openAdd("lunch"); }}
        className="fixed bottom-24 right-4 z-30 grid h-14 w-14 place-items-center rounded-full grad-accent text-background shadow-lg shadow-black/40 animate-pulse-glow"
      >
...
function ScannerView({
  ...
  const handleFile = useCallback(async (file: File) => {
    if (!file.type.startsWith("image/")) {
      setError("Fichier non supporté. Utilise une image (JPG, PNG, WEBP).");
      return;
    }
    if (file.size > 10 * 1024 * 1024) {
      setError("Image trop grande (max 10 Mo).");
      return;
    }
  ...
  const totalAdjusted = result
    ? result.aliments.reduce(
        (acc, item, i) => {
          const f = PORTION_FACTOR[portion[i] ?? "moyenne"];
          return {
            kcal:        acc.kcal        + Math.round(item.kcal        * f),
            proteines_g: acc.proteines_g + Math.round(item.proteines_g * f * 10) / 10,
            glucides_g:  acc.glucides_g  + Math.round(item.glucides_g  * f * 10) / 10,
            lipides_g:   acc.lipides_g   + Math.round(item.lipides_g   * f * 10) / 10,
          };
        },
        { kcal: 0, proteines_g: 0, glucides_g: 0, lipides_g: 0 },
      )
  ...
  const handleSave = () => {
    ...
    result.aliments.forEach((item, i) => {
      ...
      addFood({
        ...
        food_name:  item.nom,
        quantity_g: Math.round(item.quantite_g * f),
        kcal:       Math.round(item.kcal        * f),
        protein_g:  Math.round(item.proteines_g * f * 10) / 10,
        carbs_g:    Math.round(item.glucides_g  * f * 10) / 10,
        fat_g:      Math.round(item.lipides_g   * f * 10) / 10,
        source:     "ai_scan",
      });
    });
    setSaved(true);
        ...
        {preview && (
          <div className="relative aspect-[4/3] overflow-hidden rounded-2xl">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40">
              <div
                className="grid h-16 w-16 place-items-center rounded-full bg-background/80 animate-pulse-glow"
                style={{ color: "var(--accent)" }}
              >
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex gap-1 rounded-xl bg-surface-1 p-1">
          {(Object.entries(mealMeta) as [MealType, typeof mealMeta[MealType]][]).map(([type, meta]) => (
            <button
              ...
              style={{
                background: targetMeal === type ? "var(--surface-3)" : "transparent",
                color: targetMeal === type ? "var(--foreground)" : "var(--muted-foreground)",
              }}
            >
              ...
              <li key={i} className="rounded-2xl border border-border bg-surface-1 p-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      ~{Math.round(item.quantite_g * f)}g · {Math.round(item.kcal * f)} kcal ·{" "}
                      P{Math.round(item.proteines_g * f)}g
                    </p>
                  </div>
                  <span
                ...
                <div className="mt-2 flex gap-1 rounded-xl bg-surface-2 p-0.5">
                  {(["petite", "moyenne", "grande"] as const).map((size) => (
                    <button
                      key={size}
                      onClick={() => setPortion((p) => ({ ...p, [i]: size }))}
                      className="flex-1 rounded-lg py-1 text-[10px] font-medium capitalize transition"
                      style={{
                        background: (portion[i] ?? "moyenne") === size ? "var(--surface-3)" : "transparent",
                        color: (portion[i] ?? "moyenne") === size ? "var(--foreground)" : "var(--muted-foreground)",
                      }}
                    >
                      {size}
                    </button>
        ...
        <div className="sticky bottom-0 rounded-2xl border border-border bg-surface-1 p-4">
          ...
          >
            {saved ? (
              <><Check className="h-4 w-4" /> Ajouté au journal</>
            ) : isAdding ? (
              <><Loader2 className="h-4 w-4 animate-spin" /> Enregistrement…</>
            ) : (
              <><Plus className="h-4 w-4" /> Ajouter à {mealMeta[targetMeal].label}</>
            )}
          </button>
        </div>
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="grid aspect-[4/3] place-items-center rounded-2xl border-2 border-dashed border-border bg-surface-1 p-6 text-center">
        <div>
          <div
            className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-surface-2 animate-pulse-glow"
            style={{ color: "var(--accent)" }}
          >
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            L'IA identifie automatiquement les aliments et leurs macros
          </p>
        </div>
      </div>
...
function SearchView({
  ...
  userId: string | undefined;
  addFood: (f: { food_name: string; meal_type: MealType; quantity_g?: number; kcal?: number }) => void;
  isAdding: boolean;
}) {
  ...
  const handleSearch = useCallback((value: string) => {
    ...
    const local = staticFoods.filter((f) =>
      f.name.toLowerCase().includes(value.toLowerCase())
    );
    setResults(local);

    // Then enrich with AI after 600ms debounce
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        // Keep static results on AI failure
      } finally {
        setSearching(false);
      }
    }, 600);
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 rounded-xl border border-border bg-surface-1 px-3 py-2.5">
        ...
        />
        {q && (
          <button onClick={() => { setQ(""); setResults([]); }} className="text-muted-foreground hover:text-foreground text-xs">
            ✕
          </button>
        )}
      </div>
      ...
      {!q && (
        <p className="text-center text-xs text-muted-foreground py-4">
          Tape un aliment pour rechercher — l'IA enrichit les résultats automatiquement
        </p>
      )}
      ...
      {results.map((f, idx) => (
        <div
          key={`${f.name}-${idx}`}
          className="flex items-center justify-between rounded-2xl border border-border bg-surface-1 p-3"
        >
          <div>
            <p className="text-sm font-semibold">{f.name}</p>
            <p className="text-xs text-muted-foreground">
              {f.per} · P{f.p}g · G{f.c}g · L{f.f}g
            </p>
          </div>
          <div className="flex items-center gap-3">
