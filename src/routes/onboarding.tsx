    ...
    <>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Âge"><Input type="number" placeholder="28" value={form.age || ""} onChange={(e) => { const v = parseInt(e.target.value); if (!isNaN(v)) update("age", v); }} /></Field>
          <Field label="Taille (cm)"><Input type="number" placeholder="180" value={form.height || ""} onChange={(e) => { const v = parseFloat(e.target.value); if (!isNaN(v)) update("height", v); }} /></Field>
          <Field label="Poids actuel (kg)"><Input type="number" placeholder="78" step={0.1} value={form.weight || ""} onChange={(e) => { const v = parseFloat(e.target.value); if (!isNaN(v)) update("weight", v); }} /></Field>
          <Field label="Poids objectif (kg)"><Input type="number" placeholder="82" step={0.1} value={form.goalWeight || ""} onChange={(e) => { const v = parseFloat(e.target.value); if (!isNaN(v)) update("goalWeight", v); }} /></Field>
        </div>
